			...
			if (is_scrolltop || anchor != '') {
				if (is_scrolltop) {
					...
					if (scrollSection.length) {
						...
						if (_UI.menuOpened) {
							if (_UI.wwidth < _UI.mediaQuery) window.dispatchEvent(_UI.menuMobileTriggerEvent);
	...
	$owlSelector.each(function() {
		...
		$elSelector.on('initialized.owl.carousel', function(event) {
			...
			var new_time = setTimeout(function() {
				...
				if (!_UI.isMobile && !$elSelector.closest('.header-wrapper').length) navHover($elSelector.parent());
		...
		$elSelector.on('change.owl.carousel', function(event) {
			if (!_UI.isMobile) _UI.owlStopVideo(event.currentTarget);
		});
		...
		if (_UI.wwidth < _UI.mediaQuery && $(this).data('stagepadding') > 25) values['stagepadding'] = 25;
...
_UI.stickyElements = function() {
	if (!_UI.isMobile) {
		...
		var calculateOffset = function(el) {
			...
			if (_UI.adminBarHeight > 0) sideOffset += _UI.adminBarHeight;
			if ($('.menu-sticky .menu-container:not(.menu-hide)').length) {
				if ($('.menu-shrink').length) {
					sideOffset += $('.navbar-brand').data('minheight') + shrink;
				} else sideOffset += ($('body.hmenu-center').length ? $('#masthead .menu-container').outerHeight() : parseInt(_UI.menuMobileHeight));
			}
